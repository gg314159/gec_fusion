...
class EfficientChannelAttention(nn.Module):           # Efficient Channel Attention module
    def __init__(self, c, b=1, gamma=2):
        ...
        self.conv1 = nn.Conv1d(1, 1, kernel_size=k, padding=int(k/2), bias=False) #inchannel = 1,outchannel=1
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # print("input x:",x.size())# input : (n,c,h,w)
        x = self.avg_pool(x) #(n,c,h,w)->(n,c,1,1)
        # print("avg_pool(x):",x.size())
        # print("x.squeeze(-1):",x.squeeze(-1).size()) #squeeze : (n,c,1,1)->(n,c,1)
        # print("x.squeeze(-1).transpose(-1, -2):",x.squeeze(-1).transpose(-1, -2).size())#transpose : (n,c,1)->(n,1,c)
        # print("self.conv1(x.squeeze(-1).transpose(-1, -2)):",self.conv1(x.squeeze(-1).transpose(-1, -2)).size()) #不变，还是(n,1,c)
        # #因为conv1d 的输入格式是(n,c,l)c是输入channel，l是长度。
...
class ECA_ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=128): #block有两种，BasicBlock或者Bottleneck，每个block里都有一个通道注意力
...
def test():
    net = ECA_ResNet50()
    y = net(torch.randn(2, 3, 224, 224)) # (batchsize,channel,h,w)
    print(y.size())
